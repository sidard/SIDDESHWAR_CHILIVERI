...
int Tx_Buffer(char  Transfer_buffer, char Message_id, char DataBytes[])
{ 
...
void CAN_Read_fun()
{
    ...
    if(Rx_Data_MsgID_11bits == Cruise_Control_Msg_ID)/*Cruise Control*/
    {
        ...
        writeEEPROM(0, Rx_data_11bytes_Buffer_0[0]); //writing data to first memory location and then goes upto last location
        __delay_ms(20);
        writeEEPROM(1, Rx_data_11bytes_Buffer_0[1]); //writing data to first memory location and then goes upto last location
        __delay_ms(20);
        writeEEPROM(2, Rx_data_11bytes_Buffer_0[2]); //writing data to first memory location and then goes upto last location
        __delay_ms(20);
        writeEEPROM(3, Rx_data_11bytes_Buffer_0[3]); //writing data to first memory location and then goes upto last location
        __delay_ms(20);
        writeEEPROM(4, Rx_data_11bytes_Buffer_0[4]); //writing data to first memory location and then goes upto last location
        __delay_ms(20);
        writeEEPROM(5, Rx_data_11bytes_Buffer_0[5]); //writing data to first memory location and then goes upto last location
        __delay_ms(20);
        writeEEPROM(6, Rx_data_11bytes_Buffer_0[6]); //writing data to first memory location and then goes upto last location
        __delay_ms(20);
        writeEEPROM(7, Rx_data_11bytes_Buffer_0[7]); //writing data to first memory location and then goes upto last location
        __delay_ms(20);  
    } 
    ...
    {  
        ...
        RC1=  1;//LED_2 
        __delay_ms(1000);
        writeEEPROM(8, Rx_data_11bytes_Buffer_0[0]); //writing data to first memory location and then goes upto last location
        __delay_ms(20);
        writeEEPROM(9, Rx_data_11bytes_Buffer_0[1]); //writing data to first memory location and then goes upto last location
        __delay_ms(20);
        writeEEPROM(10, Rx_data_11bytes_Buffer_0[2]); //writing data to first memory location and then goes upto last location
        __delay_ms(20);
        writeEEPROM(11, Rx_data_11bytes_Buffer_0[3]); //writing data to first memory location and then goes upto last location
        __delay_ms(20);
        writeEEPROM(12, Rx_data_11bytes_Buffer_0[4]); //writing data to first memory location and then goes upto last location
        __delay_ms(20);
        writeEEPROM(13, Rx_data_11bytes_Buffer_0[5]); //writing data to first memory location and then goes upto last location
        __delay_ms(20);
        writeEEPROM(14, Rx_data_11bytes_Buffer_0[6]); //writing data to first memory location and then goes upto last location
        __delay_ms(20);
        writeEEPROM(15, Rx_data_11bytes_Buffer_0[7]); //writing data to first memory location and then goes upto last location
        __delay_ms(20);  
    }
